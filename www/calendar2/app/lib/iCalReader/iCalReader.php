...
class iCalReader {
	...
	/**
	* Parses a ics file into an array
	* @param string $filename Path to ics file
	* @return Object iCal object
	*/
	public function readFile($filename) {
		$source = $this->getFileContents($filename);
		return $this->readSource($source);
	}

	/**
	* Parses ics formated data/source into iCal object
	* @param string $source .ics formatted data
	* @return Object iCal object
	*/
	public function readSource($source)	{
		...
		if (stristr($source_lines[0], 'BEGIN:VCALENDAR') === false) {
			return false;
		} else {
			// TODO: Fix multiline-description problem (see http://tools.ietf.org/html/rfc2445#section-4.8.1.5)
			foreach ($source_lines as $line) {

				//skip blank lines, or lines with only new line chars
				if(empty($line) or $line == "\n" or $line == "\n\r") continue;

				$line = trim($line);
				$add  = $this->keyValueFromString($line);
				if ($add === false) {
					$line = str_replace('\N', "<br />", str_replace('\,', ",", str_replace('\n', "<br />", $line)));
					$this->addCalendarComponentWithKeyAndValue($type, false, $line);
					continue;
				} 

				list($keyword, $value) = $add;
				$value = str_replace('\N', "<br />", str_replace('\,', ",", str_replace('\n', "<br />", $value)));
